...
1. メンバーに呼びかけ、 `!join` によるゲーム参加を促します。自身の `!join` も忘れずに。  
   ...
   また、ゲーム開始前は以下のような設定項目を変更することができます。
   - `!set_cycles` によって周回数（ひとつの本に対し各プレイヤーが何回記入するか）を設定することができます。
   - `!set_channel` によって全体への通知メッセージを送信するテキストチャンネルを設定することができます。
...
| 周回数設定 | `!set_cycles 1` |
| 公開チャンネル設定 | `!set_channel 123456789123456789` |
| メンバー再読み込み | `!reload_member` |
| ゲーム開始 | `!start_game` |
| ゲーム強制終了 | `!quit_game` |
| 本文執筆開始 | `!start_script` |
| ターン終了 | `!next_turn` |
...
`!set_channel`
- 全体に公開する通知メッセージが送信されるテキストチャンネルを設定します。  
  `!join` や `!start_game` などのコマンドの通知メッセージがここで設定されたチャンネルに送信されるようになります。
- コマンド文字列 `!set_channel` のあとに半角スペースを挟んで設定するテキストチャンネルのIDを打ち込みます。  
  たとえば、`123456789123456789` というIDのチャンネルに設定にしたい場合のコマンドは以下の通りです。  
  ```
  !set_cycles 123456789123456789
  ```

- 数字以外の文字を指定するとエラーメッセージを返します。

#### メンバー再読み込み

`!reload_member`
- 当botから見えるアカウント（以下メンバーと呼びます）一覧を再読み込みします。  
  当botはメンバー以外にはDMを送信できません。
